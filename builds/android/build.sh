...
(android_build_verify_so "libzyre.so" &> /dev/null) || {
    ...
    (bash ${ZYRE_ROOT}/builds/android/build.sh) || exit 1
    UPSTREAM_PREFIX=${ZYRE_ROOT}/builds/android/prefix/${TOOLCHAIN_NAME}
    cp -r ${UPSTREAM_PREFIX}/* ${ANDROID_BUILD_PREFIX}
}

##
# Build demidi from local source

(android_build_verify_so "libdemidi.so" "libzyre.so" &> /dev/null) || {
    rm -rf "${cache}/demidi"
    (cp -r ../.. "${cache}/demidi" && cd "${cache}/demidi" \
        && make clean && rm -f configure config.status)

    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'

    (cd "${cache}/demidi" && ./autogen.sh \
...
android_build_verify_so "libdemidi.so" "libzyre.so"
